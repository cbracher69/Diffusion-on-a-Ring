...
/* *** VARIABLE CLASSES ***/
	...
	/* Explicit Constructor - by Value */
		Pattern(unsigned long long Value)
		{
			Code = Value;
		}
	...
	/* Increment Position of Element nu,
	   Function Returns Type of Jump (A,B,C,D = 0,1,2,3) */
		int Increment(int nu)
		{
			...
			unsigned long long Aux1 = (Code & exp2pow[(Aux + p - 1) % p]);
			unsigned long long Aux2 = (Code & exp2pow[(Aux + 2) % p]);
	...
	/* Decrement Position of Element nu
	   Function Returns Type of Jump (A,B,C,D = 0,1,2,3) */
		int Decrement(int nu)
		{
			...
			unsigned long long Aux1 = (Code & exp2pow[(Aux + 1) % p]);
			unsigned long long Aux2 = (Code & exp2pow[(Aux + p - 2) % p]);
	...
	/* Check For Matching Pattern */
		...
		/* Create Separate Codes For Atoms And Holes in Subpattern */
			unsigned long long AtomCode = 0;
			unsigned long long HoleCode = 0;
			string Aux;
...
/* Auxiliary Field:  Powers of 2 */
	exp2pow = new unsigned long long[64];
	exp2pow[0] = 1;
	for (i = 1; i < 64; ++i)
	{
...
/* Permitted Range of Sites: 3...63 */
	if (p < 3)
		ErrorMessage("At least three sites required");
	if (p > 63)
		ErrorMessage("At most 63 sites permitted");
